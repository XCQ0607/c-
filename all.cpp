...
class Basic{
    ...
    public:
	void a();
	unsigned int a1(unsigned int shift);
    void b();
    void c();
    void d();
    };
...
void Basic::a(){
	...
	*/
    ...
    //例如，如果你有一个u_sign类型的变量，并且你将 -50 赋值给它，C++ 会将 -50 转换为一个无符号整数。具体来说，它会将 -50 加上无符号整数类型的最大值加 1（对于 32 位无符号整数，这个值是 2^32），然后取模运算的结果。
    //这里就是2^32-50
    ...
    cout << "float非科学计数法:" << a << endl;
    double b = 1234567890.1234567890;
    cout << "double非科学计数法:" << b << endl;
